...
function scene:create( event )
	...
	local function onBtnListener( event )
		...
		elseif (id == "addTripBtn") then
			local options = { time = 200, effect = "flipFadeOutIn"}
			composer.gotoScene("myTripV4",options)				
		end
		return true
	end
	local path = system.pathForFile( "tripPlan.txt", system.DocumentsDirectory )
...
------------------- 旅程計畫元件 -------------------
	...
	-- 按鈕-新增旅程計畫
		local addTripBtn = widget.newButton({
			...
			font = getFont.font,
			fontSize = 14,
			width =  titleBase.contentWidth*0.5,
			height =  ceil(79*0.4),
			onRelease = onBtnListener,
		})
	...
	-- 顯示文字-提示文字
		...
		hintText.anchorX = 0
		hintText.anchorY = 0
		hintText.x = backBtn.x
		hintText.y = titleBase.y+titleBase.contentHeight*0.5+ceil(60*hRate)
	-- 計畫列表
		-- 陰影
		local planBaseTopShadow = display.newImage( sceneGroup, "assets/s-up.png", cx, hintText.y+ceil(50*hRate))
		planBaseTopShadow.anchorY = 1
		planBaseTopShadow.width = titleBase.contentWidth
		planBaseTopShadow.height = floor(planBaseTopShadow.height*0.3)
		local endNum = 10
		for i=1,endNum do
			-- 白底
			local planBase = display.newRect( sceneGroup, cx, hintText.y+ceil(110*hRate)*0.5, titleBase.contentWidth, ceil(120*wRate))
			planBase.anchorY = 0
			...
			sceneGroup:insert(tripPointText)
			...
			sceneGroup:insert(startDateText)
			...
			sceneGroup:insert(backDateText)
			...
			if (i == endNum) then
			-- 陰影
				local planBaseBottomShadow = display.newImage( sceneGroup, "assets/s-down.png", cx, planBase.y+planBase.contentHeight)
...
function scene:hide( event )
	...
	if ( phase == "will" ) then
	elseif ( phase == "did" ) then
		composer.removeScene("myTripPlan")
	end
end
