...
local nickName
if ( composer.getVariable("userNickName") ) then
	nickName = composer.getVariable("userNickName")
else
	nickName = "登入/註冊"
end
...
function scene:create( event )
	...
	-- 登入或編輯監聽事件
		local function loginOrEditation( event )
			if ( event.phase == "ended" ) then
				if ( isLogin == false ) then
					...
					composer.gotoScene("LN_welcome", options)
				else
					local options = { 
						effect = "fromLeft", 
						time = 300,
					}
					composer.gotoScene("PC_personalEdit",options)
		...
		nameText:addEventListener( "touch", loginOrEditation )
		pencilPic:addEventListener( "touch", loginOrEditation )
	-- 優惠碼
		...
		couponBase:addEventListener("touch", 
			function (event)
				...
				elseif (phase == "ended") then
					couponText:setFillColor(1)
					couponNumText:setFillColor(1)
					composer.showOverlay("PC_coupon", { isModal = true, effect = "fromLeft", time = 300} )
				end
				return true
			end
		)
		...
		pointBase:addEventListener("touch", 
			function (event)
				...
				elseif (phase == "ended") then
					...
					composer.showOverlay( "PC_point", options)
				end
				return true
			end
		)
	...
	-- 我的訂單按鈕
		local myOrderBtn = widget.newButton({
			...
			onRelease = function(event)
				composer.gotoScene( "PC_orderList", { effect = "fade", time = 200 } )
				return true
			end,
		}) 
	...
	-- 願望清單按鈕
		local hopeListBtn = widget.newButton({
			...
			fillColor = { default = { 1, 1, 1, 1 }, over = { 1, 1, 1, 1 } },
			onRelease = function(event)
				composer.gotoScene( "PC_hopelist",{effect = "fade", time = 200} )
				return true
			end,
		}) 
		...
		local accSettingBtn = widget.newButton({
			...
			onRelease = function()
				if ( isLogin == true ) then
					composer.showOverlay( "PC_accountSetting",	{ isModal = true, effect = "fade", time = 200 } )
				end
				return true
			end,
		}) 
	...
	-- 按鈕
		local chgLanguageBtn = widget.newButton({
			...
			fillColor = { default = { 1, 1, 1, 1 }, over = {204/255, 204/255, 204/255, 0.2 } },
			onRelease = function()
				composer.showOverlay( "PC_somethingChanged", { isModal = true, effect = "fromLeft", time = 200} )
				return true
			end,
		}) 
						...
						function (event)
							if ( event.action == "clicked" ) then
								...
								if ( index == 1) then
									...
									local function revokeTokenListener( event )
										if ( event.isError ) then
											...
											if ( decodedData["success"] ) then
												composer.setVariable( "accessToken", false )
												isLogin = false
												composer.setVariable( "isLogin", false )
												composer.setVariable( "userNickName", false )
												nameText.text = "登入/註冊"
												edit.fill = { type = "image", filename = "assets/user.jpg" }
												if ( composer.getVariable("shoppingCartScene") ) then
									...
									local revokeTokenUrl = optionsTable.revokeTokenUrl..composer.getVariable("accessToken")
									network.request( revokeTokenUrl, "POST", revokeTokenListener)
								end
							end
						end )
				end
...
function scene:hide( event )
	...
	elseif phase == "did" then
		composer.removeScene("PC_personalCenter")
	end
end
