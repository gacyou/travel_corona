...
---------------------------------------------------------------------------------------------------
	...
	local circlePad = 14
	for i=1,picNum do
		point[i] = display.newCircle(welcomeGroup, 0, (screenH*0.875)+oy, 5)
...
---------------------------------------------------------------------------------------------------
	...
	titleBackgroundShadow.width = (titleBackgroundShadow.width*0.33)+ox+ox
	titleBackgroundShadow.height = (titleBackground.height*1.05)
	...
	local titleText = display.newText({
		text = "旅伴系統",
		y = titleBackground.y,
		font = native.newFont("Font_Data/DFTT_NR9"),
		fontSize = 16,
	})
...
function scene:hide( event )
    ...
    elseif ( phase == "did" ) then
        ...
        composer.removeScene("travelPartner")
    end
end
