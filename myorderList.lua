...
local function onBtnListener( event )
	...
	if( targetId == "infoBtn") then 
		composer.showOverlay("orderInfomation",
			{ isModal = true, effect = "zoomOutIn", time = 300})
	end
...
---------------------------------------------------------------------------------------------------
	...
	local grayLine = display.newLine(-ox,-oy+ceil(178*hRate),screenW+ox,-oy+ceil(178*hRate))
...
-----------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if ( isLogin == false ) then 
		...
		historyOrderGroup:insert(funBirdPic)
		...
		historyOrderGroup:insert(text1)
		...
		historyOrderGroup:insert(goShoppingBtn)
		goShoppingBtn.anchorX = 0
		goShoppingBtn.x = cx
		goShoppingBtn.y = -oy+ceil(cy*1.15)
		...
		historyOrderGroup:insert(text2)
		text2:setFillColor(unpack(wordColor))
		text2.anchorX = 0
		text2.x = cx+goShoppingBtn.contentWidth
		text2.y = -oy+ceil(cy*1.15)
	else
