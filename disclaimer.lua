...
function scene:create( event )
	...
	-- 免責聲明讀檔	
		...
		--end
	-- 建立免責聲明
		local disclaimerWhiteBase = display.newRect( cx, titleBase.y+(titleBase.contentHeight*0.5)+floor(40*hRate), screenW+ox+ox, titleBase.contentHeight)
		sceneGroup:insert(disclaimerWhiteBase)
		disclaimerWhiteBase.x = cx
		disclaimerWhiteBase.y = disclaimerWhiteBase.y+(disclaimerWhiteBase.contentHeight*0.5)
	...
	-- 免責聲明scrollview
		local scrollView = widget.newScrollView({
			id = "scrollView",
			top = disclaimerWhiteBase.y+disclaimerWhiteBase.contentHeight*0.5,
			left = -ox,
			width = screenW+ox+ox,
			height = screenH+oy-(disclaimerWhiteBase.y),
	...
	-- 免責說明內容	
		...
		if ( scrollViewHeight > scrollView.contentHeight ) then
