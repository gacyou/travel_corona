...
function scene:create( event )
	...
	local function scrollViewSlide( event )
		...
		if (phase == "moved") then
			...
			if (name == "accommodation") then
				...
				local dx = math.abs(event.xStart-event.x)
				if (dx > 10) then
					scrollView["accommodation"]:takeFocus(event)
				end
			end
...
-- 網址解析	
	local sliderUrl = {}
	--local baseSliderUrl = "http://traveltest.1758play.com:8080/img/"
	local urlGetFileNamePattern = "%a+://%w+%.%w+[%.%w]*%:?%d*[/%w]*/(.+%.%a+)"
	-- local urlPaten = "%a+%:%/%/%w+%.%w+%.%w+%:?%d*%/%w+%/([%w%.%a]+)"
	for i=1,5 do
...
-- 固定時間換頁
	...
	--local cover = display.newRect(cx,cy,screenW+ox+ox,screenH+oy+oy)
	...
	for i=1,#sliderUrl do
		local function netWorkListener( event )
			...
			if (event.phase == "ended" ) then
				picCount = picCount + 1 
				progressView:setProgress(picCount/#sliderUrl)
				print( "Displaying response TitleImage file " ..i)
...
-- 浮動廣告頁
	-- 陰影
	-- 陰影內部白框寬是原寬的0.96875，高是原高的0.98
	shadow["advertisementMain"] = display.newImageRect("assets/shadow-3.png", 1080, 1920)
	bkgScrollViewGroup:insert(shadow["advertisementMain"])
	shadow["advertisementMain"].width = ceil(1039*wRate)+ox+ox
	shadow["advertisementMain"].height = floor(447*wRate)
	...
	--base["advertisementMain"]:setFillColor(unpack(separateLineColor))
------------------- 熱門目的地 -------------------
-- 顯示文字-"熱門目的地"
...
-- 熱門目的地API
	local function hotSpotListener( event )
		if ( event.isError ) then
			...
			for i=1,#decodedData do
				...
				base["hotSpot"].id = decodedData[i]["id"]
				base["hotSpot"].name = "hotSpot"
				base["hotSpot"]:addEventListener("touch",scrollViewSlide)
				if (i == #decodedData) then 
	...
	local hotSpotUrl = "http://211.21.114.208/1.0/GetTopDestinationList"
...
-- 顯示文字-"獨家主題"
	...
	text["uniqueTheme"].y = scrollView["hotSpot"].y+scrollView["hotSpot"].contentHeight+ceil(65*hRate)
-- 顯示文字-"查看全部"
...
-- API 前四個
	...
	local function uniqueThemeListener( event )
		if ( event.isError ) then
			print( "Network error!")
		else
			--print ("RESPONSE: "..event.response)
			...
			for i=1, #decodedData do
				...
				-- 增加scrollview width的邊界
					if (i == #decodedData) then 
						...
						uniqueThemeSrollWidth = shadow["uniqueThemePadding"].x+shadow["uniqueThemePadding"].contentWidth
						scrollView["uniqueTheme"]:setScrollWidth(uniqueThemeSrollWidth+10)
					end
				-- 白底
					...
					base["uniqueTheme"].id = decodedData[i]["id"]
					base["uniqueTheme"].name = "uniqueTheme"
					base["uniqueTheme"]:addEventListener("touch",scrollViewSlide)
				-- 顯示的圖片位置
				...
				-- 圖片url
					picUrl["uniqueTheme"] = decodedData[i]["exTopicImgs"][1].url
					local function getUniqueThemePic( event )
						if (event.isError) then 
							print("Network error - download failed")
						end
						if (event.phase == "ended" ) then
							-- 顯示圖片
							print( "Displaying response uniqueTheme file " ..i)
							local uniqueThemePic = display.newImage(event.response.filename, event.response.baseDirectory)
							uniqueThemeGroup:insert(uniqueThemePriceTagNum[i],uniqueThemePic)
							uniqueThemePic.x = base["uniqueThemePic"][i].x
							uniqueThemePic.y = base["uniqueThemePic"][i].y
							uniqueThemePic.width = base["uniqueThemePic"][i].contentWidth
							uniqueThemePic.height = base["uniqueThemePic"][i].contentHeight
						end
					end
					local uniqueThemePicName = string.match(picUrl["uniqueTheme"],urlGetFileNamePattern)
					network.download(
						picUrl["uniqueTheme"],
						"GET",
						getUniqueThemePic,
				...
				-- 顯示文字-行程抬頭
					...
					if (string.len(text["uniqueThemeTitle"].text) > 48) then
						text["uniqueThemeTitle"].text = text["uniqueThemeTitle"].text:sub(1,48).."..."
					end
				-- 顯示文字-行程內文
					...
					if (string.len(text["uniqueThemeContent"].text) > 60) then
						text["uniqueThemeContent"].text = text["uniqueThemeContent"].text:sub(1,60).."..."
					end
				-- 圖示-地點
				...
				-- 價位標籤
					...
					uniqueThemePriceTagNum[i] = uniqueThemeGroup.numChildren
				-- 顯示文字-幣別
	...
	local uniqueThemeUrl = "http://211.21.114.208/1.0/GetExclusiveTopicList"
	network.request(uniqueThemeUrl, "POST", uniqueThemeListener, parms)
------------------- 熱門行程 -------------------
-- 顯示文字-"熱門行程"
...
-- 熱門行程ScrollVIew元件
	local function hotJourneyListener( event )
		if ( event.isError ) then
			print( "Network error!")
        else
			--print ("RESPONSE: "..event.response)
			...
			for i=1,#decodedData do
				-- 陰影
					...
					shadow["hotJourney"].x = 0+(shadow["hotJourney"].contentWidth+ceil(22*wRate))*(i-1)
					...
					if (i == #decodedData) then 
						...
						hotJourneySrollWidth = shadow["hotJourneyPadding"].x+shadow["hotJourneyPadding"].contentWidth
						scrollView["hotJourney"]:setScrollWidth(hotJourneySrollWidth+10)
					end
				-- 白底
					...
					base["hotJourney"].id = decodedData[i]["id"]
					base["hotJourney"].name = "hotJourney"
					base["hotJourney"]:addEventListener("touch",scrollViewSlide)
				-- 圖片位置
				...
				-- 圖片url
					picUrl["hotJourney"] = decodedData[i]["exTopicImgs"][1].url
					local function getHotJourneyPic( event )
						if (event.isError) then 
							print("Network error - download failed")
						end
						if (event.phase == "ended" ) then
							-- 顯示圖片
							print( "Displaying response hotJourney file " ..i)
							local hotJourneyPic = display.newImage(event.response.filename, event.response.baseDirectory)
							hotJourneyGroup:insert(hotJourneyPriceTagNum[i],hotJourneyPic)
							hotJourneyPic.x = base["hotJourneyPic"][i].x
							hotJourneyPic.y = base["hotJourneyPic"][i].y
							hotJourneyPic.width = base["hotJourneyPic"][i].contentWidth
							hotJourneyPic.height = base["hotJourneyPic"][i].contentHeight
						end
					end
					local uniqueThemePicName = string.match(picUrl["hotJourney"],urlGetFileNamePattern)
					network.download(
						picUrl["hotJourney"],
						"GET",
						getHotJourneyPic,
				...
				-- 顯示文字-行程抬頭
					...
					if (string.len(text["hotJourneyTitle"].text) > 48) then
						text["hotJourneyTitle"].text = text["hotJourneyTitle"].text:sub(1,48).."..."
					end
				-- 顯示文字-行程內文
					...
					if (string.len(text["hotJourneyContent"].text) > 60) then
						text["hotJourneyContent"].text = text["hotJourneyContent"].text:sub(1,60).."..."
					end
				-- 圖示-地點
				...
				-- 價位標籤
					...
					hotJourneyPriceTagNum[i] = hotJourneyGroup.numChildren
				-- 顯示文字-幣別
	...
	local hotJourneyUrl = "http://211.21.114.208/1.0/GetExclusiveTopicList"
	network.request(hotJourneyUrl, "POST", hotJourneyListener, parms)
------------------- 推薦住宿 -------------------
-- 顯示文字-"推薦住宿"
	text["accommodation"] = display.newText({
		parent = bkgScrollViewGroup,
		text = "推薦住宿",
		font = getFont.font,
		fontSize = 14,
	})
	text["accommodation"]:setFillColor(unpack(mainColor1))
	text["accommodation"].anchorX = 0
	text["accommodation"].anchorY = 0
	text["accommodation"].x = ceil(22*wRate)
	text["accommodation"].y = scrollView["hotJourney"].y+scrollView["hotJourney"].contentHeight+ceil(65*hRate)
-- 顯示文字-"查看全部"
	text["accommodationAll"] = display.newText({
		parent = bkgScrollViewGroup,
		text = "查看全部",
		font = getFont.font,
		fontSize = 14,
	})
	text["accommodationAll"]:setFillColor(unpack(wordColor))
	text["accommodationAll"].anchorX = 1
	text["accommodationAll"].anchorY = 0
	text["accommodationAll"].x = scrollView["background"].contentWidth-ceil(30*wRate)
	text["accommodationAll"].y = text["accommodation"].y
-- 推薦住宿ScrollView
	scrollView["accommodation"] = widget.newScrollView({
		id = "accommodationScrollview",
		width = screenW+ox+ox,
		height = floor(338*wRate),
		verticalScrollDisabled = true,
		--isBounceEnabled = false,
	...
	bkgScrollViewGroup:insert(scrollView["accommodation"])
	scrollView["accommodation"].anchorX = 0
	scrollView["accommodation"].anchorY = 0
	scrollView["accommodation"].x = ceil(22*wRate)
	scrollView["accommodation"].y = text["accommodation"].y+text["accommodation"].contentHeight+ceil(30*hRate)
-- 推薦住宿ScrollView元件
	local function accommodationListener( event )
		if ( event.isError ) then
			print( "Network error!")
        else
			--print ("RESPONSE: "..event.response)
			...
			--	print(k,v)
			--end
			local accommodationGroup = display.newGroup()
			scrollView["accommodation"]:insert(accommodationGroup)
			local accommodationSrollWidth = 0
			for i=1,#decodedData do
				-- 背景
				base["accommodation"] = display.newRect(accommodationGroup, 0+(ceil(358*wRate)+ceil(10*wRate))*(i-1), 0, ceil(358*wRate), scrollView["accommodation"].contentHeight)
				base["accommodation"]:setFillColor(math.random(),math.random(),math.random())
				base["accommodation"].anchorX = 0
				base["accommodation"].anchorY = 0 
				base["accommodation"].id = decodedData[i]["id"]
				base["accommodation"].name = "accommodation"
				base["accommodation"]:addEventListener("touch",scrollViewSlide)
				if (i == #decodedData) then 
					base["accommodationPadding"] = display.newRect(accommodationGroup, base["accommodation"].x+base["accommodation"].contentWidth, 0, ceil(22*wRate),base["accommodation"].contentHeight)
					base["accommodationPadding"].anchorX = 0
					base["accommodationPadding"].anchorY = 0
					accommodationSrollWidth = base["accommodationPadding"].x+base["accommodationPadding"].contentWidth
					scrollView["accommodation"]:setScrollWidth(accommodationSrollWidth+10)
				end
				-- 黑色文字底層
				shadow["accommodation"] = display.newImageRect("assets/index-mask.png",357,84)
				accommodationGroup:insert(shadow["accommodation"])
				shadow["accommodation"].anchorX = 0
				shadow["accommodation"].anchorY = 1
				shadow["accommodation"].x = base["accommodation"].x
				shadow["accommodation"].y = base["accommodation"].contentHeight
				shadow["accommodation"].width = base["accommodation"].contentWidth
				shadow["accommodation"].height = ceil(shadow["accommodation"].height*wRate)
				-- 顯示文字-熱門目的地地名
				text["accommodation"] = display.newText({
					parent = accommodationGroup,
					x = base["accommodation"].x+base["accommodation"].contentWidth*0.5,
					y = shadow["accommodation"].y-shadow["accommodation"].contentHeight*0.3,
					text = decodedData[i]["title"],
					font = getFont.font,
					fontSize = 12,
				})
	...
	parms.headers = headers
	parms.body = body
	local accommodationUrl = "http://211.21.114.208/1.0/GetTopDestinationList"
	network.request( accommodationUrl, "POST", accommodationListener, parms)
------------------- 頁尾廣告 -------------------
-- 頁尾廣告ScrollView
	...
	scrollView["advertisementBottom"].y = scrollView["accommodation"].y+scrollView["accommodation"].contentHeight+ceil(85*hRate)
...
-- 頁尾廣告ScrollView元件
	...
	for i=1,3 do
		...
		base["advertisementBottom"]:addEventListener("touch",scrollViewSlide)
	end
end
...
function scene:show( event )
		...
		scrollView["background"]:scrollTo("top", { time = 0 , onComplete = function ()
			scrollView["hotSpot"]:scrollTo("left", { time = 0 , onComplete = function ()
				scrollView["uniqueTheme"]:scrollTo("left", { time = 0, onComplete = function ()
					scrollView["hotJourney"]:scrollTo("left", { time = 0, onComplete = function ()
						 scrollView["accommodation"]:scrollTo("left", { time = 0 })
					end })
				end })
			end })
		end})
	...
	elseif ( phase == "did" ) then
		if (isStart == true and isReturn == false) then
			isReturn = true
			textFiled["search"].isVisible = true
