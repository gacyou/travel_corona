...
function scene:create( event )
	...
	-- 圖示-edit.png
		...
		editPic:addEventListener( "touch", 
			function ( event )
	...
	-- 圖示-性別
		local genderPic
		if ( infoList[5]:match(pattern) == "男" ) then
	...
	-- 顯示文字-年齡
		local setAge
		if ( ptnrList[2]:match(pattern) ) then
			setAge = "年齡："..ptnrList[2]:match(pattern).."歲"
		else
			setAge = "年齡：別問，我會害羞"
		end
	...
	-- 顯示文字-洲別
		local continentInfoText = display.newText({
			text = "洲別："..infoList[6]:match(pattern),
			font = getFont.font,
			fontSize = 12,
		})
	...
	-- 顯示文字-國家
		local countryInfoText = display.newText({
			text = "國家："..infoList[7]:match(pattern),
			font = getFont.font,
			fontSize = 12,
		})
	...
	-- 顯示文字-通用口語
		local setLang
		if ( ptnrList[6]:match(pattern) ) then
			setLang = "通用口語："..ptnrList[6]:match(pattern)
		else
			setLang = "通用口語：無法用言語表達"
		end
	...
	-- 顯示文字-喜歡旅行方式
		local setTripInfo
		if ( ptnrList[7]:match(pattern) ~= "請選擇" ) then
			setTripInfo = "喜歡的旅行方式："..ptnrList[7]:match(pattern)
		else
			setTripInfo = "喜歡的旅行方式：我想想"
		end
	...
	-- 顯示文字-關於我內容
		local setAboutMe
		if ( ptnrList[9]:match(pattern) ) then
			setAboutMe = ptnrList[9]:match(pattern)
		else
			setAboutMe = ""
		end
...
function scene:hide( event )
	...
	if ( phase == "will" ) then
	elseif ( phase == "did" ) then
		composer.removeScene("TP_myInformation")
	end
end
