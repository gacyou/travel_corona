...
local titleListBoundary, titleListGroup, sortListBoundary, sortListGroup
...
function scene:create( event )
	...
	-- 排序按鈕
	 	local sortBtn = widget.newButton({
			...
			if ( setStatus["sortList"] == "down" ) then
					if (setStatus["titleArrow"] == "up") then
		...
		local function showList( event )
			...
			if (phase == "ended") then
				if (setStatus["titleArrow"] == "down") then
					if (setStatus["sortList"] == "up") then
						setStatus["sortList"] = "down"
						sortListBoundary.isVisible = false
						transition.to( sortListGroup, { y = -titleListBoundary.contentHeight, time = 200})
					end
					titleArrow:rotate(180)
	...
	-- 排序下拉式選單
		local sortListText = {}
		sortListBoundary = display.newContainer( screenW+ox+ox, screenH*0.25 )
		sceneGroup:insert(sortListBoundary)
		sortListBoundary.anchorY = 0
		sortListBoundary.x = cx
		sortListBoundary.y = titleBaseShadow.y+titleBaseShadow.contentHeight*0.5
		sortListGroup = display.newGroup()
		sortListBoundary:insert(sortListGroup)
		local sortListShadow = display.newImageRect( sortListGroup, "assets/shadow-320480.png", screenW+ox+ox, sortListBoundary.contentHeight )
		local sortListBase = display.newRect( sortListGroup, 0, -sortListBoundary.contentHeight*0.5, sortListShadow.contentWidth*0.95, sortListShadow.contentHeight*59/60 )
	...
	-- 下拉式選單元件
		...
		for i = 1, #sortOptions do
			sortListBase = display.newRect( sortListGroup, 0, -sortListBoundary.contentHeight*0.5, sortListBoundary.contentWidth*0.95, screenH/16)
			sortListBase.anchorY = 0
...
function scene:hide( event )
	...
	if ( phase == "will" ) then
	elseif ( phase == "did" ) then
		composer.removeScene("HS_otherTitle")
	end
end
