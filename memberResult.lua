...
local getGender
if (composer.getVariable("getGender")) then
	getGender = composer.getVariable("getGender")
else
	getGender = "鬼"
end

local getLanguage
if (composer.getVariable("getLanguage")) then
	getLanguage = composer.getVariable("getLanguage")
else
	getLanguage = "鬼話"
end
...
local function onBtnListener( event )
	...
	if (id == "backBtn") then
		...
		composer.gotoScene("memberSearch",options)
	end
	if (id == "listBtn") then
...
-- 下拉式選單按鈕
 	...
 	sceneGroup:insert(listBtn)
 	listBtn.width = listBtn.width*0.07
 	listBtn.height = listBtn.height*0.07
 	listBtn.anchorX = 1
 	listBtn.x = (screenW+ox)-(49*wRate)
 	listBtn.y = titleBase.y
 	setStatus["listBtn"] = "down"
...
------------------- 搜尋結果 -------------------
 	...
 	-- 沒有結果
 		local resultText = display.newText({
				text = getGender.."/"..getLanguage.."/"..getCountry.." ",
				font = getFont.font,
				fontSize = 12,
			})
		...
		resultText.y = titleBase.y+(titleBase.contentHeight/2)+ceil(50*hRate)
	...
	-- 顯示圖像的ScrollView
		local resultScrollView = widget.newScrollView({
				id = "resultScrollView",
				width = screenW+ox+ox,
				height = screenH+oy+oy-(resultText.y+resultText.contentHeight),
		...
		resultScrollView.y = resultText.y+resultText.contentHeight
	-- 圖像排序
		...
		local function tapToIntrodunction(event)
			...
			composer.gotoScene("partnerIntroduction",options)
		end
		for i=1, resultNum do
			xBasement = ((i-1)%3)
...
function scene:hide( event )
    ...
    if ( phase == "will" ) then
	elseif ( phase == "did" ) then
		composer.removeScene("partnerResult")
		getGender = nil
		getLanguage = nil
		getCountry = nil
	end
