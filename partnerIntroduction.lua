...
local partnerName = composer.getVariable("partnerName")
...
local function onBtnListener( event )
	...
	if (id == "listBtn") then
		if (setStatus["listBtn"] == "up") then 
			...
			transition.to(titleListGroup,{ y = -240, time = 300})
			timer.performWithDelay(300, function() titleListGroup.isVisible = false ;end)
			timer.performWithDelay(300, function() titleListBoundary.isVisible = false ;end)
		else
			setStatus["listBtn"] = "up"
			titleListGroup.isVisible = true
			titleListBoundary.isVisible = true
			transition.to(titleListGroup,{ y = 0, time = 300})
		end
	end
end

local function titleListListener( event )
	local phase = event.phase
	if( event.target.name ~= "titleSelectionBase") then
		if (phase == "began" or phase == "moved") then
			if (nowTarget == nil and prevTarget == nil) then 
				-- 第一次觸碰選項
				nowTarget = event.target
				nowTarget:setFillColor(unpack(subColor1))
			...
			if (event.target ~= nowTarget) then
				...
				prevTarget:setFillColor(unpack(wordColor))
				nowTarget = event.target
				nowTarget:setFillColor(unpack(subColor1))
			end
		end
		if( phase == "ended" or phase == "cancelled") then 
			setStatus["listBtn"] = "down"
			transition.to(titleGroup,{ y = -240, time = 300})
			timer.performWithDelay(300, function() titleGroup.isVisible = false ; end)
			timer.performWithDelay(300, function() titleSelection.isVisible = false ; end)
			timer.performWithDelay(300, function() nowTarget:setFillColor(unpack(wordColor)) 
				nowTarget = nil
				prevTarget = nil
				end)
			if(listOptions[event.target.id] ~= nil) then
				timer.performWithDelay(300, function() composer.gotoScene(listOptions[event.target.id]) ; end)
			end
		end
	else
		-- 移動到字體外當作取消選擇
		if(phase == "ended" or phase == "cancelled") then
			if (math.floor(event.x) < 130 or math.floor(event.x) > 190) then
				setStatus["listBtn"] = "down"
				transition.to(titleGroup,{ y = -240, time = 300})
				nowTarget:setFillColor(unpack(wordColor))
				timer.performWithDelay(300, function() titleGroup.isVisible = false ; end)
				timer.performWithDelay(300, function() titleSelection.isVisible = false ; end)
				timer.performWithDelay(300, function() nowTarget = nil	prevTarget = nil ; end)
			end
		end
	end
	return true
...
------------------- 第一區塊背景元件 -------------------
-- 藍底
	local blueBase = display.newImageRect("assets/bg-personalcenter.png",1080,450)
...
-- 圖示-talk.png
	local messagePic = display.newImageRect("assets/talk.png", 148, 148)
	sceneGroup:insert(messagePic)
	messagePic.width = ceil(messagePic.width*0.2)
	messagePic.height = ceil(messagePic.height*0.2)
	messagePic.anchorX = 1 
	messagePic.anchorY = 1
	messagePic.x = (screenW+ox)-ceil(30*wRate)
	messagePic.y = -oy+titleBase.contentHeight+blueBase.contentHeight-floor(30*hRate)
-- 圖示-save.png
	local myFavoritePic = display.newImageRect("assets/save.png", 148, 148)
	sceneGroup:insert(myFavoritePic)
	myFavoritePic.width = ceil(myFavoritePic.width*0.2)
	myFavoritePic.height = ceil(myFavoritePic.height*0.2)
	myFavoritePic.anchorX = 1
	myFavoritePic.anchorY = 1
	myFavoritePic.x = messagePic.x-messagePic.contentWidth-ceil(22*wRate)
	myFavoritePic.y = messagePic.y
-- 我的最愛監聽事件
	local function addFavorite(event)
		-- body
		if (isCollected == nil) then 
			myFavoritePic.fill = { type = "image", filename = "assets/save-.png"}
			isCollected = true
		else
			myFavoritePic.fill = { type = "image", filename = "assets/save.png"}
			isCollected = false
	...
	myFavoritePic:addEventListener("tap",addFavorite)
-- 顯示文字-用戶姓名
...
-- 圖示-性別
	...
	genderPic.anchorX = 0
	genderPic.x = nameText.x+nameText.contentWidth+ceil(10*wRate)
	genderPic.y = nameText.y+(nameText.contentHeight/2)
-- 分隔線1
...
function scene:hide( event )
    ...
    elseif ( phase == "did" ) then
        -- Code here runs immediately after the scene goes entirely off screen
 		composer.removeScene("partnerIntroduction")
    end
end
