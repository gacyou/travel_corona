...
local getRegion
if (composer.getVariable("getRegion")) then 
	getRegion = composer.getVariable("getRegion")
else
	getRegion = "鬼地方"
end
...
------------------- 搜尋結果 -------------------
	...
	resultText.y = titleBase.y+(titleBase.contentHeight/2)+(50*hRate)+(resultText.contentHeight/2)
	local resultNum = 7
...
-- 圖像排序
	...
	local function tapToIntrodunction(event)
		...
		composer.gotoScene("selfIntroduction",options)
	end
	for i=1, resultNum do
		xBasement = ((i-1)%3)
